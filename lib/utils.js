...
export function useDashboardData() {
  const { data: pageOne, error: pageOneError } = useSWR(
    `https://api.nomics.com/v1/currencies/ticker?key=${process.env.NEXT_PUBLIC_NOMICS_API_KEY}&convert=USD&status=active&per-page=100&page=1`,
    fetcher,
  );
  const { data: pageTwo, error: pageTwoError } = useSWR(
    `https://api.nomics.com/v1/currencies/ticker?key=${process.env.NEXT_PUBLIC_NOMICS_API_KEY}&convert=USD&status=active&per-page=100&page=2`,
    fetcher,
  );
  ...
  if (holdings?.holdings && pageOne?.length && pageTwo?.length) {
    // const partialCurrencies = currencies.slice(0, 1500);
    const partialCurrencies = [...pageOne, ...pageTwo];
    console.log(partialCurrencies.length);
  ...
  if (pageOneError || pageTwoError) {
    isError = { type: 'currencies', error: pageOneError || pageTwoError };
